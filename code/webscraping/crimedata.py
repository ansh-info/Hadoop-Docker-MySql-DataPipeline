...
def get_all_headers():
    ...
    return columns, len(header_elements)

def scrape_table(columns, num_headers):
    # Locate the table
    ...
    for row in rows:
        ...
        if len(row_data) > len(columns):
            ...
            for i in range(additional_columns):
                columns.append(f"Extra_Column_{i+1 + len(columns) - num_headers}")
...
columns, num_headers = get_all_headers()
scrape_table(columns, num_headers)
...
if data:
    ...
    local_file_path = 'Crimes_2001_to_Present_data.csv'
